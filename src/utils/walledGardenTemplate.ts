...
<head>
  ...
  <title>Service Suspended • Walled Garden Isolation | ${companyName}</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .container {
      ...
      border-radius: 24px;
      padding: 32px 24px;
      box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5), 0 0 40px rgba(225, 29, 72, 0.15);
      text-align: center;
      position: relative;
      overflow: hidden;
    }
    ...
    .icon-shield {
      width: 64px;
      height: 64px;
      background: linear-gradient(135deg, rgba(225, 29, 72, 0.25), rgba(15, 23, 42, 0.9));
...
<body>
  <div class="container">
    ...
    <div class="highlight-box">
      <div class="title">Official Payment Channels</div>
      <div class="payment-grid">
        <div class="payment-card">
          <span class="payment-brand gcash">GCash Official</span>
          <span class="acc-num">${gcashNumber}</span>
          <span class="acc-name">${gcashName}</span>
        </div>
        <div class="payment-card">
          <span class="payment-brand maya">Maya Official</span>
          <span class="acc-num">${mayaNumber}</span>
          <span class="acc-name">${mayaName}</span>
        </div>
        ${
          bankName && bankAccountNumber
            ? `<div class="payment-card" style="grid-column: 1 / -1;">
          <span class="payment-brand bank">${bankName} Bank Transfer</span>
          <span class="acc-num">${bankAccountNumber}</span>
          <span class="acc-name">${bankAccountName || gcashName}</span>
        </div>`
            : ''
        }
      </div>
