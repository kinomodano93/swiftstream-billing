...
export const PaymentList: React.FC<PaymentListProps> = ({
  ...
  const xenditTotal = payments.filter((p) => p.paymentMethod === 'xendit').reduce((s, p) => s + p.amount, 0);
      ...
      {activeSubTab === 'webhooks' ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            ...
            <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800">
              <span className="text-[10px] text-slate-400 font-medium">🏦 Bank / Check</span>
              <h4 className="text-base sm:text-lg font-bold text-slate-200 mt-1">{formatCurrency(bankTotal)}</h4>
            </div>
          </div>
