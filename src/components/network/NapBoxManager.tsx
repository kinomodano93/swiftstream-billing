...
export const NapBoxManager: React.FC<NapBoxManagerProps> = ({ onSelectCustomer }) => {
  ...
  const handleCreateBox = (e: React.FormEvent) => {
    ...
    addNapBox({
      ...
      feedSourceType,
      upstreamNapId: feedSourceType === 'nap' ? (upstreamNapId || undefined) : undefined,
      fbtRatio,
      plcSplitterType,
      totalPorts,
      fiberCoreColor,
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleCreateBox} className="p-6 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3 p-3 rounded-2xl bg-purple-950/20 border border-purple-800/40">
                <div>
                  <label className="block text-purple-300 mb-1 font-medium flex items-center gap-1">
                    <Server className="w-3.5 h-3.5 text-purple-400" />
                    <span>Parent Serving OLT POP *</span>
                  </label>
                  <select
                    value={selectedOltId}
                    onChange={(e) => {
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-purple-800/60 rounded-xl text-slate-100 font-semibold focus:outline-none focus:border-purple-400"
                  >
                    {oltNodes.map((olt) => (
                ...
                <div>
                  <label className="block text-purple-300 mb-1 font-medium flex items-center gap-1">
                    ...
                    <span>OLT PON Port Number *</span>
                  </label>
                  <select
                    value={ponPortNumber}
                    onChange={(e) => setPonPortNumber(Number(e.target.value))}
                    className="w-full px-3 py-2 bg-slate-950 border border-purple-800/60 rounded-xl text-slate-100 font-mono font-bold focus:outline-none focus:border-purple-400"
                  >
                    {Array.from(
              ...
              <div className="p-3 rounded-2xl bg-purple-950/30 border border-purple-800/50 space-y-2.5">
                <div className="flex items-center justify-between">
                  <label className="text-purple-300 font-semibold text-xs flex items-center gap-1.5">
                    <Waypoints className="w-3.5 h-3.5 text-purple-400" />
                    <span>Cascade Feed Parent (Span Origin) *</span>
                  </label>
                  <span className="text-[10px] text-slate-400 font-mono">Asymmetric Bus Run</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!upstreamNapId && candidates.length > 0) {
                        setUpstreamNapId(candidates[candidates.length - 1].id);
                      }
                    }}
                    className={`p-2.5 rounded-xl border text-left text-xs font-semibold transition-all ${
                      feedSourceType === 'nap'
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>Upstream NAP (Hop)</span>
                    </div>
                    <span className="text-[10px] text-slate-400 font-normal block mt-1">
                      Daisy-chained from previous box
                    </span>
                  </button>
                </div>
                ...
                {feedSourceType === 'nap' && (
                  <div className="pt-1">
                    <label className="block text-cyan-300 text-[11px] mb-1 font-medium">
                    ...
                    {(() => {
                      ...
                      if (candidates.length === 0) {
                        return (
                          <div className="p-2 rounded-xl bg-amber-950/40 border border-amber-800/50 text-[11px] text-amber-300 flex items-center gap-2">
                            <AlertTriangle className="w-4 h-4 shrink-0 text-amber-400" />
                            <span>No existing NAP boxes on PON #{ponPortNumber}. This will be the first feeder box.</span>
                          </div>
                        );
                      }
                      ...
                      return (
                        <select
                          value={upstreamNapId || candidates[candidates.length - 1]?.id}
                          onChange={(e) => setUpstreamNapId(e.target.value)}
                          className="w-full px-3 py-2 bg-slate-950 border border-cyan-800/60 rounded-xl text-slate-100 font-mono text-xs focus:outline-none focus:border-cyan-400"
                        >
                          {candidates.map((b) => (
                            <option key={b.id} value={b.id}>
                              {b.code} - {b.name} (Brgy. {b.barangay})
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-400 mb-1 font-medium">Barangay Coverage *</label>
                  <select
                  ...
                  {isCustomBarangay && (
                    <input
                      ...
                      className="w-full mt-2 px-3 py-1.5 bg-slate-950 border border-cyan-500/50 rounded-xl text-slate-100 text-xs focus:outline-none focus:border-cyan-400"
                    />
                  )}
                </div>
              </div>
              ...
              <div className="px-3 py-1.5 rounded-xl bg-cyan-950/40 border border-cyan-900/50 flex items-center justify-between text-[11px] text-cyan-300">
                <span className="flex items-center gap-1.5">
                  <Compass className="w-3.5 h-3.5 text-cyan-400" />
                  <span>GPS auto-aligned to <strong>Brgy. {isCustomBarangay ? customBarangay || 'Custom' : barangay}</strong></span>
                </span>
                <span className="text-slate-400 text-[10px]">Fine-tune for exact pole</span>
              </div>

              {/* FBT Coupler & PLC Splitter Selectors */}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-amber-300 mb-1 font-medium flex items-center justify-between text-xs">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-amber-600/50 rounded-xl text-slate-100 font-mono text-xs font-bold focus:outline-none focus:border-amber-400"
                  >
                ...
                <div>
                  <label className="block text-cyan-300 mb-1 font-medium flex items-center justify-between text-xs">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-cyan-800/60 rounded-xl text-slate-100 font-mono text-xs font-bold focus:outline-none focus:border-cyan-400"
                  >
              ...
              {(() => {
                ...
                const upstreamNap = feedSourceType === 'nap'
                  ? napBoxes.find((b) => b.id === (upstreamNapId || candidates[candidates.length - 1]?.id))
                  : null;
                ...
                const oltDirectSpan =
                  targetOlt && typeof latitude === 'number' && typeof longitude === 'number' && !isNaN(latitude) && !isNaN(longitude)
                    ? calculateSpanMetrics(targetOlt.latitude, targetOlt.longitude, latitude, longitude)
                    : null;
                ...
                const arrivingInputDbm = upstreamNap
                  ? Number((baseTx - 3.2 - hopFiberLoss).toFixed(2)) // Estimated cascaded power from upstream
                ...
                return (
                  <div className="p-3 rounded-2xl bg-gradient-to-r from-purple-950/40 via-slate-950 to-cyan-950/40 border border-purple-800/60 space-y-2.5 font-mono text-xs">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <div className="flex items-center gap-2">
                        <span className="p-1 rounded bg-purple-900/60 text-purple-300 font-bold text-[10px]">
                    ...
                    {oltDirectSpan && feedSourceType === 'nap' && (
                      <div className="pt-1.5 border-t border-slate-900 flex items-center justify-between text-[10px] text-slate-400">
                        <span>Cumulative Air Run from OLT:</span>
                        <span className="text-purple-300 font-bold">{oltDirectSpan.formattedDirect}</span>
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    disabled
                    value={totalPorts}
                    className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-xl text-slate-300 font-mono font-bold cursor-not-allowed"
                  />
                  <span className="text-[10px] text-slate-500 mt-1 block">Synced with {plcSplitterType} PLC</span>
                </div>
