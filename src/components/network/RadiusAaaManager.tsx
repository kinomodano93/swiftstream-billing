...
export const RadiusAaaManager: React.FC = () => {
  ...
  const [radiusServers, setRadiusServers] = useState<RadiusServerConfig[]>(() => {
    try {
      const saved = localStorage.getItem('swiftstream_radius_servers_v4');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('swiftstream_radius_servers_v4', JSON.stringify(radiusServers));
    } catch (_) {}
  }, [radiusServers]);

  // Clean RADIUS Users (radcheck / radreply)
  const [radiusUsers, setRadiusUsers] = useState<RadiusUser[]>(() => {
    try {
      const saved = localStorage.getItem('swiftstream_radius_users_v4');
      if (saved) return JSON.parse(saved);
    } catch (_) {}
    return [];
  });

  // Clean Active Accounting Sessions (radacct)
  const [sessions, setSessions] = useState<RadiusSession[]>(() => {
    try {
      const saved = localStorage.getItem('swiftstream_radius_sessions_v4');
      if (saved) return JSON.parse(saved);
    } catch (_) {}
    return [];
  });
                ...
                {sessions.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-slate-500">
                      No active RADIUS accounting sessions currently reporting from NAS gateways.
                    </td>
                  </tr>
                ) : (
                  sessions.map((sess) => (
                ...
                {radiusUsers.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center text-slate-500">
                      No RADIUS users configured. Click "+ Add RADIUS User" or link subscribers from Subscribers CRM.
                    </td>
                  </tr>
                ) : (
      ...
      {activeTab === 'servers' && (
        radiusServers.length === 0 ? (
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-8 text-center text-slate-500 text-xs">
            ...
            <p className="text-slate-300 font-semibold">No RADIUS Servers Configured</p>
            <p className="text-[11px] text-slate-500 mt-0.5">
              No external FreeRADIUS authentication or accounting nodes registered.
            </p>
          </div>
        ) : (
